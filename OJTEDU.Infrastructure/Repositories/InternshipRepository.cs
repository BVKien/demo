...
bool isDescending)
        {
            ...
            else if (role == "Dean")
            {
                ...
                {
                    throw new KeyNotFoundException("Dean not found or department not active.");
                }
                ...
                // Lọc Internship theo MajorId trong Department
                query = query.Where(i => i.MajorId.HasValue && majorIdsInDepartment.Contains(i.MajorId.Value));
            }
            else
            {
                throw new UnauthorizedAccessException("Role not authorized to view internships.");
