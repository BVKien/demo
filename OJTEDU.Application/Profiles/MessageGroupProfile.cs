...
namespace OJTEDU.Application.Profiles
{
    public class MessageGroupProfile : Profile
    {
        public MessageGroupProfile()
        {
            ...
            CreateMap<MessageGroup, GetAllMemberForUserAsyncForAdminDOETDeanLecturerMentorDTO>()
                .ReverseMap();

            CreateMap<MessageGroup, GetAllGroupForUserAsyncForAdminDOETDeanLecturerMentorDTO>()
                .ForMember(dest => dest.GroupName, opt => opt.MapFrom(src => src.GroupChat.GroupName))
                .ReverseMap();
